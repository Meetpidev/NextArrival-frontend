...
export default function AdminDashboardView() {
  ...
  const fetchPartnerRequests = async () => {
    ...
    try {
      const { data } = await adminApi.partnerRequests();
      setPartnerRequests(Array.isArray(data?.data?.requests) ? data.data.requests : []);
    } catch (e) {
      console.error("Partner requests fetch error:", e);
    } finally {
  ...
  const fetchContactInquiries = async () => {
    ...
    try {
      const { data } = await adminApi.contactInquiries();
      setContactInquiries(Array.isArray(data?.data?.inquiries) ? data.data.inquiries : []);
    } catch (e) {
      console.error("Contact inquiries fetch error:", e);
    } finally {
  ...
  const handleModerateContactInquiry = async (inquiryId: string, status: "REVIEWED" | "RESOLVED" | "REJECTED") => {
    ...
    try {
      ...
      console.error("Contact inquiry update error:", e);
    } finally {
      setProcessingAction(null);
    }
  };
  ...
  const handleModeratePartnerRequest = async (requestId: string, status: "REVIEWED" | "RESOLVED" | "REJECTED") => {
    ...
    try {
      ...
      console.error("Partner request update error:", e);
    } finally {
      setProcessingAction(null);
    }
  };
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === "partnerRequests" && (
                <div className="space-y-6 text-xs text-slate-600">
                  <div className="flex justify-between items-center border-b border-slate-200 pb-3">
                    <h2 className="text-base font-bold text-slate-900">Partner Requests</h2>
                    <div className="flex space-x-2">
                      <button
                        onClick={() => setPartnerRequestsShowHistory(false)}
                        className={`px-3 py-1.5 font-bold transition-all text-xs rounded-lg cursor-pointer ${!partnerRequestsShowHistory ? "bg-[#cfa052] text-white" : "bg-white text-slate-500 hover:text-slate-900 border border-slate-200"}`}
                      >
                        Pending Requests
                      </button>
                      <button
                        onClick={() => setPartnerRequestsShowHistory(true)}
                        className={`px-3 py-1.5 font-bold transition-all text-xs rounded-lg cursor-pointer ${partnerRequestsShowHistory ? "bg-[#cfa052] text-white" : "bg-white text-slate-500 hover:text-slate-900 border border-slate-200"}`}
                      >
                        History
                      </button>
                  ...
                  {loadingPartnerRequests ? (
                    <div className="flex justify-center py-16"><Loader2 className="h-8 w-8 animate-spin text-[#cfa052]" /></div>
                  ) : partnerRequests.filter((req) =>
                    partnerRequestsShowHistory ? req.status !== "PENDING" : req.status === "PENDING"
                  ).length === 0 ? (
                    <div className="text-center py-16 bg-white border border-slate-200 rounded-xl italic text-slate-400">
                      No partner requests found in this category.
                    </div>
                  ) : (
                    <div className="space-y-6">
                      {partnerRequests
                        ...
                        .map((req) => (
                          <div key={req.id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
                            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                              <div>
                                <span className="font-bold text-sm text-slate-900">{req.organizationName}</span>
                                <span className="text-slate-400 ml-2">({req.fullName})</span>
                              </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                              <div className="space-y-2.5">
                                <p><strong className="text-slate-400 uppercase text-[9px] tracking-wider">Email:</strong> {req.email}</p>
                                <p><strong className="text-slate-400 uppercase text-[9px] tracking-wider">Phone:</strong> {req.phone || "—"}</p>
                                <p><strong className="text-slate-400 uppercase text-[9px] tracking-wider">Country:</strong> {req.country}</p>
                                <p><strong className="text-slate-400 uppercase text-[9px] tracking-wider">City:</strong> {req.cityRegion || "—"}</p>
                                <p><strong className="text-slate-400 uppercase text-[9px] tracking-wider">Goal:</strong> {req.partnershipGoal}</p>
                              </div>

                              <div className="space-y-3 bg-slate-50 border border-slate-200 p-4 rounded-xl">
                                <p className="font-semibold text-slate-700">Tell Us More</p>
                                <p className="text-slate-500 text-[10px] leading-relaxed">{req.tellUsMore}</p>
                                <p className="text-slate-400 text-[10px]">Submitted {new Date(req.createdAt).toLocaleString()}</p>
                              </div>
                            </div>
                            ...
                            {req.status === "PENDING" && (
                              <div className="flex gap-2 justify-end">
                                <button
                                  onClick={() => handleModeratePartnerRequest(req.id, "REJECTED")}
                                  disabled={processingAction === req.id}
                                  className="rounded-lg border border-red-200 bg-red-50 text-red-600 px-4 py-2 font-bold hover:bg-red-100 transition-colors flex items-center gap-1 cursor-pointer"
                                >
                                  <X className="h-3.5 w-3.5" />
              ...
              {activeTab === "contactInquiries" && (
                <div className="space-y-6 text-xs text-slate-600">
                  <div className="flex justify-between items-center border-b border-slate-200 pb-3">
                    <h2 className="text-base font-bold text-slate-900">Contact Inquiries</h2>
                    <div className="flex space-x-2">
                      <button
                        onClick={() => setContactInquiriesShowHistory(false)}
                        className={`px-3 py-1.5 font-bold transition-all text-xs rounded-lg cursor-pointer ${!contactInquiriesShowHistory ? "bg-[#cfa052] text-white" : "bg-white text-slate-500 hover:text-slate-900 border border-slate-200"}`}
                      >
                        Pending
                      </button>
                      <button
                        onClick={() => setContactInquiriesShowHistory(true)}
                        className={`px-3 py-1.5 font-bold transition-all text-xs rounded-lg cursor-pointer ${contactInquiriesShowHistory ? "bg-[#cfa052] text-white" : "bg-white text-slate-500 hover:text-slate-900 border border-slate-200"}`}
                      >
                        History
                      </button>
                  ...
                  {loadingContactInquiries ? (
                    <div className="flex justify-center py-16"><Loader2 className="h-8 w-8 animate-spin text-[#cfa052]" /></div>
                  ) : contactInquiries.filter((req) =>
                    contactInquiriesShowHistory ? req.status !== "PENDING" : req.status === "PENDING"
                  ).length === 0 ? (
                    <div className="text-center py-16 bg-white border border-slate-200 rounded-xl italic text-slate-400">
                          ...
                          <div key={req.id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
                            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                              <div>
                                ...
                                <span className="text-slate-400 ml-2">({req.email})</span>
                              </div>
                              <span className={`px-2 py-0.5 rounded-lg font-bold uppercase text-[10px] ${req.status === "PENDING"
                                ? "bg-amber-50 border border-amber-200 text-amber-700"
                                : req.status === "REVIEWED"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                              <div className="space-y-2.5">
                                <p><strong className="text-slate-400 uppercase text-[9px] tracking-wider">Who:</strong> {req.whoAreYou}</p>
                                <p><strong className="text-slate-400 uppercase text-[9px] tracking-wider">Destination City:</strong> {req.destinationCity || "—"}</p>
                                <p><strong className="text-slate-400 uppercase text-[9px] tracking-wider">Visa Status:</strong> {req.visaStatus || "—"}</p>
                                <p><strong className="text-slate-400 uppercase text-[9px] tracking-wider">Subject:</strong> {req.subject}</p>
                              </div>

                              <div className="space-y-3 bg-slate-50 border border-slate-200 p-4 rounded-xl">
                                <p className="font-semibold text-slate-700">Message Detail</p>
                                <p className="text-slate-500 text-[10px] leading-relaxed">{req.messageDetail}</p>
                                <p className="text-slate-400 text-[10px]">Submitted {new Date(req.createdAt).toLocaleString()}</p>
                              </div>
                            </div>
